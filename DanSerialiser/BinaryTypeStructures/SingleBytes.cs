...
namespace DanSerialiser.BinaryTypeStructures
{
	...
	internal struct SingleBytes
	{
		...
		public SingleBytes(float value)
		{
			this = default(SingleBytes); // Have to do this to avoid "Field 'Byte{x}' must be fully assigned before control is returned to the caller" errors
			this.Value = value;
		}

		public SingleBytes(byte[] littleEndianBytes)
		{
			...
			this = default(SingleBytes); // Have to do this to avoid "Field 'Value' must be fully assigned before control is returned to the caller" error
		...
		{
			if (BitConverter.IsLittleEndian)
				return new[] { (byte)BinarySerialisationDataType.Single, Byte0, Byte1, Byte2, Byte3 };
			else
				return new[] { (byte)BinarySerialisationDataType.Single, Byte3, Byte2, Byte1, Byte0 };
		}
	}
}