	...
	internal sealed class BinarySerialisationReaderTypeReader
	{
		...
		public object Read(BinarySerialisationReader reader, BinarySerialisationDataType nextEntryType, bool ignoreAnyInvalidTypes)
		{
			...
			foreach (var field in _fields)
			{
				...
				var value = reader.Read(ignoreAnyInvalidTypes);
		...
		{
			public FieldSettingDetails(int fieldNameReferenceID, Action<object, object>[] setters)
			{
			...
			/// one entry and if this field relates to deprecated property/ies in some way then this may multiple entries
			/// </summary>
